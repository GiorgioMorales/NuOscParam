...
class ref_mass_splitting_3flavor:
    def __init__(self):
        ...
        self.delta_m21_NO_sigma = 0.21e-5
        self.delta_m32_NO_sigma = 0.032e-3
        ...
        self.delta_m21_IO_sigma = 0.21e-5
        self.delta_m32_IO_sigma = 0.032e-3
    ...
    def get_rnd_mass_NO(self, nstat=1):
        ...
        return np.array([zero, ran12, ran32 + ran12]).T()
    ...
    def get_rnd_gauss_mass_NO(self, nstat=1):
        ran12 = np.random.normal(self.delta_m21_NO, self.delta_m21_NO_sigma, nstat)
        ran32 = np.random.normal(self.delta_m32_NO, self.delta_m32_NO_sigma, nstat)
        zero = np.zeros_like(ran12)
        return np.array([zero, ran12, ran32 + ran12])

    def get_rnd_gauss_mass_IO(self, nstat=1):
        ran12 = np.random.normal(self.delta_m21_IO, self.delta_m21_IO_sigma, nstat)
        ran32 = np.random.normal(self.delta_m32_IO, self.delta_m32_IO_sigma, nstat)
        zero = np.zeros_like(ran12)
        return np.array([zero, ran12, ran32 + ran12])
...
class ref_PMNS_3flavor:
    ...
    def get_rnd_gauss_angle_NO(self, nstat=1):
        ran12 = np.random.normal(self.theta12_NO, self.theta12_NO_sigma, nstat)
        ran13 = np.random.normal(self.theta13_NO, self.theta13_NO_sigma, nstat)
        ran23 = np.random.normal(self.theta23_NO, self.theta23_NO_sigma, nstat)
        randcp = np.random.normal(self.delta_cp_NO, self.delta_cp_NO_sigma, nstat)
        return np.array([ran12, ran23, ran13, randcp])

    def get_rnd_gauss_angle_IO(self, nstat=1):
        ran12 = np.random.normal(self.theta12_IO, self.theta12_IO_sigma, nstat)
        ran13 = np.random.normal(self.theta13_IO, self.theta13_IO_sigma, nstat)
        ran23 = np.random.normal(self.theta23_IO, self.theta23_IO_sigma, nstat)
        randcp = np.random.normal(self.delta_cp_IO, self.delta_cp_IO_sigma, nstat)
        return np.array([ran12, ran23, ran13, randcp])
...
def check_U_prior_constaint(U, U_min, U_max):
    # sel0 = np.all(U[:,0:3,0:3]>Uzero.reshape(1,3,3), axis=(1, 2))

    # sel = (np.all(U[sel0,0:3,0:3]<U_max.reshape(1,3,3), axis=(1, 2)) &
    #    np.all(U[sel0,0:3,0:3]>U_min.reshape(1,3,3), axis=(1, 2)))
    # print(sel)
